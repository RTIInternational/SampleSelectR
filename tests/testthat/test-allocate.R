...
test_that("Proportional - proportions in returned sample match the relative proportions in N.h (when min(n.samp*N.h/sum(N.h)) > lbound and all((n.samp*N.h/sum(N.h)) %% 1 == 0)", {
  ...
  expect_equal(
    allocate("proportional", N.h = c(2, 4), n.samp = 3, lbound = 1),
    c(1, 2)
  )
  expect_equal(
    allocate("proportional", N.h = c(5, 5), n.samp = 4, lbound = 1),
    c(2, 2)
  )
  expect_equal(
    allocate("proportional", N.h = c(20, 30, 40), n.samp = 9, lbound = 2),
    c(2, 3, 4)
  )
  expect_equal(
    allocate("proportional", N.h = c(25, 50, 100), n.samp = 21, lbound = 3),
    c(3, 6, 12)
  )
})
...
test_that("Proportional - does sum(rounded_allocations) = n.samp?", {
  expect_equal(
    sum(allocate("proportional", N.h = c(2, 4), n.samp = 3, lbound = 1)),
    3
  )
  expect_equal(
    sum(allocate("proportional", N.h = c(5, 5), n.samp = 4, lbound = 1)),
    4
  )
  expect_equal(
    sum(allocate("proportional", N.h = c(20, 30, 40), n.samp = 9)),
    9
  )
  expect_equal(
    sum(allocate("proportional", N.h = c(25, 50, 100), n.samp = 21, lbound = 3)),
    21
  )
  expect_equal(
    sum(allocate("proportional", N.h = c(25, 50, 100), n.samp = 9, lbound = 3)),
    9
  )
})
...
test_that("Proportional - does min(rounded_allocations) >= lbound?", {
  expect_gte(
    min(allocate("proportional", N.h = c(2, 4), n.samp = 3, lbound = 1)),
    1
  )
  expect_gte(
    min(allocate("proportional", N.h = c(5, 5), n.samp = 4, lbound = 1)),
    1
  )
  expect_gte(
    min(allocate("proportional", N.h = c(20, 30, 40), n.samp = 9)),
    2
  )
  expect_gte(
    min(allocate("proportional", N.h = c(25, 50, 100), n.samp = 21, lbound = 3)),
    3
  )
  expect_gte(
    min(allocate("proportional", N.h = c(25, 50, 100), n.samp = 9, lbound = 3)),
    3
  )
})
...
test_that("Power - proportions in returned sample match the unrounded sample (when min(n.samp*N.h/sum(N.h)) > lbound and expected unrounded sample are all integers)", {
  ...
  expect_equal(
    allocate("power", power = 0.25, N.h = c(16, 81), n.samp = 10),
    c(4, 6)
  )
  expect_equal(
    allocate("power", power = 1 / 3, N.h = c(8, 27), n.samp = 15),
    c(6, 9)
  )
  expect_equal(
    allocate("power", power = 1 / 2, N.h = c(9, 16, 25), n.samp = 24),
    c(6, 8, 10)
  )
})
...
test_that("Power - does sum(rounded_allocations) = n.samp?", {
  expect_equal(
    sum(allocate("power", power = 0.5, N.h = c(4, 4, 16), n.samp = 20)),
    20
  )
  expect_equal(
    sum(allocate("power", power = 0.25, N.h = c(16, 81), n.samp = 10)),
    10
  )
  expect_equal(
    sum(allocate("power", power = 1 / 3, N.h = c(8, 27), n.samp = 15)),
    15
  )
  expect_equal(
    sum(allocate("power", power = 1 / 2, N.h = c(9, 16, 25), n.samp = 24)),
    24
  )
  expect_equal(
    sum(allocate("power", power = 1 / 2, N.h = c(9, 16, 25), n.samp = 24, lbound = 8)),
    24
  )
})
...
test_that("Power - does min(rounded_allocations) >= lbound?", {
  expect_gte(
    min(allocate("power", power = 0.5, N.h = c(4, 4, 16), n.samp = 20)),
    2
  )
  expect_gte(
    min(allocate("power", power = 0.25, N.h = c(16, 81), n.samp = 10)),
    2
  )
  expect_gte(
    min(allocate("power", power = 1 / 3, N.h = c(8, 27), n.samp = 15)),
    2
  )
  expect_gte(
    min(allocate("power", power = 1 / 2, N.h = c(9, 16, 25), n.samp = 24)),
    2
  )
  expect_gte(
    min(allocate("power", power = 1 / 2, N.h = c(9, 16, 25), n.samp = 24, lbound = 8)),
    8
  )
})
...
test_that("Neyman - proportions in returned sample match the unrounded sample (when min(n.samp*N.h/sum(N.h)) > lbound and expected unrounded sample are all integers)", {
  ...
  expect_equal(
    allocate("neyman", N.h = c(8, 8, 32), n.samp = 20, S.h = c(1, 1, 2)),
    c(2, 2, 16)
  )
  expect_equal(
    allocate("neyman", N.h = c(10, 10, 10), n.samp = 20, S.h = c(1, 1, 2)),
    c(5, 5, 10)
  )
  expect_equal(
    allocate("neyman", N.h = c(30, 60), n.samp = 12, S.h = c(1, 1)),
    c(4, 8)
  )
  expect_equal(
...
test_that("Neyman - does sum(rounded_allocations) = n.samp?", {
  expect_equal(
    sum(allocate("neyman", N.h = c(8, 8, 32), n.samp = 20, S.h = c(1, 1, 2))),
    20
  )
  expect_equal(
    sum(allocate("neyman", N.h = c(10, 10, 10), n.samp = 20, S.h = c(1, 1, 2))),
    20
  )
  expect_equal(
    sum(allocate("neyman", N.h = c(30, 60), n.samp = 12, S.h = c(1, 1))),
    12
  )
  expect_equal(
    sum(allocate("neyman", N.h = c(25, 50, 100), n.samp = 99, S.h = c(5, 15, 2.5), lbound = 1)),
    99
  )
  expect_equal(
    sum(allocate("neyman", N.h = c(100, 50, 150), n.samp = 100, S.h = c(7.5, 10, 2.5), lbound = 25)),
    100
  )
})
...
test_that("Neyman - does min(rounded_allocations) >= lbound?", {
  expect_gte(
    min(allocate("neyman", N.h = c(8, 8, 32), n.samp = 20, S.h = c(1, 1, 2))),
    2
  )
  expect_gte(
    min(allocate("neyman", N.h = c(10, 10, 10), n.samp = 20, S.h = c(1, 1, 2))),
    2
  )
  expect_gte(
    min(allocate("neyman", N.h = c(30, 60), n.samp = 12, S.h = c(1, 1))),
    2
  )
  expect_gte(
    min(allocate("neyman", N.h = c(25, 50, 100), n.samp = 99, S.h = c(5, 15, 2.5), lbound = 1)),
    1
  )
  expect_gte(
    min(allocate("neyman", N.h = c(100, 50, 150), n.samp = 100, S.h = c(7.5, 10, 2.5), lbound = 25)),
    25
  )
})
...
test_that("Optimal (cost) - proportions in returned sample match the unrounded sample (when min(n.samp*N.h/sum(N.h)) > lbound and expected unrounded sample are all integers)", {
  ...
  expect_equal(
    allocate("optimal", N.h = c(100, 150), S.h = c(1.5, 1), c.h = c(1, 4), cost = 24),
    c(8, 4)
  )
  expect_equal(
    allocate("optimal", N.h = c(100, 75, 150), S.h = c(1, 1, 1.5), c.h = c(1, 1, 4), cost = 100),
    c(16, 12, 18)
  )
})
...
test_that("Optimal (cost) - does the total cost of the allocated sample align with the cost parameter (when a solution exists (e.g., (sum(lbound*c.h) > cost)?", {
  expect_equal(
    sum(c(1, 4) * allocate("optimal", N.h = c(100, 150), S.h = c(1.5, 1), c.h = c(1, 4), cost = 24)),
    24
  )
  expect_equal(
    sum(c(1, 1, 4) * allocate("optimal", N.h = c(100, 75, 150), S.h = c(1, 1, 1.5), c.h = c(1, 1, 4), cost = 100)),
    100
  )
})
...
test_that("Optimal (cost) - does min(rounded_allocations) >= lbound?", {
  expect_gte(
    min(allocate("optimal", N.h = c(100, 150), S.h = c(1.5, 1), c.h = c(1, 4), cost = 24, lbound = 4)),
    4
  )
  expect_gte(
...
test_that("Optimal (variance) - does min(rounded_allocations) >= lbound?", {
  expect_gte(
    min(allocate("optimal", N.h = c(36, 64), S.h = c(9, 9), c.h = c(4, 1), variance = 1)),
    2
  )
  expect_gte(
